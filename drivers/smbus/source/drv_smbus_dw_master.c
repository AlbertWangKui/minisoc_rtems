 ...
 *          device locking, and error handling around the core address assignment logic.
 * @param[in] devId SMBus device identifier
...
S32 smbusArpEnumDev(DevList_e devId)
{
    ...
    LOGI("%s: Starting ARP device enumeration\n", __func__);
    ...
    while (!enumerationComplete && deviceCount < SMBUS_MAX_DEVICE_NUM) {
        ...
        LOGI("%s: Device %d configured successfully - addr=0x%02X, vendor=0x%04X\n",
    ...
    LOGI("%s: ARP enumeration completed - %d devices discovered\n",
...
unlock:
    ...
    LOGI("%s: ARP enumeration finished with result: %d\n", __func__, ret);
    return ret;
}
...
S32 smbusReset(DevList_e devId)
{
    ...
    /* ===== 7. Success Path ===== */
    LOGI("SMBus I2C Reset: API reset completed successfully for device %d\n", devId);
    LOGI("SMBus I2C Reset: Device restored to %s mode\n",
         (pDrvData->pSmbusDev.mode == DW_SMBUS_MODE_MASTER) ? "MASTER" : "SLAVE");
