 ...
 * @return  EXIT_SUCCESS成功 / -EINVAL参数无效 / -EIO读取失败
 */
static S32 sm2DevCfgGet(DevList_e devId, SbrSm2Cfg_s *cfg)
{
    ...
    LOGI("%s-%d: SBR dump - regAddr:%p, irqNo:%u, irqPrio:%u\r\n",
 ...
 * @param[in] sm2Drv 驱动数据结构
 * @return  EXIT_SUCCESS成功 / -EINVAL参数无效
 */
 ...
 * @param[in] sm2Drv 驱动数据结构
 * @return  EXIT_SUCCESS成功 / -EINVAL参数无效 / -EIO操作失败
 */
...
static void sm2IrqHandler(void *arg)
{
    ...
    if (reg->irqStatus.dword != 0) {
        LOGD("%s-%d: SM2 IRQ STATUS 0x%x\r\n", __func__, __LINE__, reg->irqStatus.dword);
        LOGD("%s-%d: SM2 Result 0x%x\r\n", __func__, __LINE__, reg->state.fields.result);
 ...
 * @brief   轮询SM2硬件空闲状态
 * @param[in] sm2Drv 驱动数据结构
 * @return  EXIT_SUCCESS硬件空闲 / -EINVAL参数无效 / -EIO硬件繁忙
 */
 ...
 * @brief   轮询SM2操作完成标志
 * @param[in] sm2Drv 驱动数据结构
 * @return  EXIT_SUCCESS操作完成 / -EINVAL参数无效 / -EIO操作未完成
 */
 ...
 * @return  EXIT_SUCCESS准备成功 / -EINVAL参数无效 / -EIO硬件忙或操作失败
 */
static S32 sm2VerifyPrepare(Sm2DrvData_s *pDrvData, U32 *signBuf, U32 *pkey, U32 *dataIn)
{
    ...
    if (ret != EXIT_SUCCESS) {
        LOGE("%s-%d: sm2PollIdle failed, ret = %d\r\n", __func__, __LINE__, ret);
        return -EIO;
    }

    if (pDrvData->sbrCfg.regAddr == NULL) {
        LOGE("%s-%d: Invalid register address\r\n", __func__, __LINE__);
        return -EINVAL;
    }
...
S32 sm2Init(DevList_e devId)
{
    ...
    if (devLockByDriver(devId, SM2_LOCK_TIMEOUT_MS) != EXIT_SUCCESS) {
        LOGE("%s-%d: Failed to lock device, devId=%d\r\n", __func__, __LINE__, devId);
        ret = -EBUSY;
        goto exit;
    }
    ...
    if(pDrvData == NULL) {
        LOGE("%s-%d: SM2 alloc memory failed\r\n", __func__, __LINE__);
        ret = -ENOMEM;
        goto unlock;
    }
    ...
    if (ret == OSP_RESOURCE_IN_USE) {
        LOGW("%s-%d: irq handler already installed\r\n", __func__, __LINE__);
        ...
        if(ret != OSP_SUCCESSFUL) {
            LOGE("%s-%d: irq priority set failed\r\n", __func__, __LINE__);
            ret = -EIO;
            goto remove_isr;
        }
    } else {
        LOGE("%s-%d: irq handler install failed\r\n", __func__, __LINE__);
        ret = -EIO;
        goto free_mem;
    }
...
S32 sm2DoVerify(DevList_e devId, U32 *signBuf, U32 *pkey, U32 *dataIn)
{
    ...
    ret = funcRunBeginHelper(devId, DRV_ID_STARS_SM2, (void **)&pDrvData);
    if (ret != EXIT_SUCCESS) {
        LOGE("%s-%d: funcRunBeginHelper failed, devId=%d, ret=%d\r\n", __func__, __LINE__, devId, ret);
        return ret;
    }
    ...
    if (ret != EXIT_SUCCESS) {
        LOGE("%s-%d: sm2VerifyPrepare failed, ret = %d\r\n", __func__, __LINE__, ret);
        goto end;
    }

    ret = sm2InterruptEnable(pDrvData);
    if (ret != EXIT_SUCCESS) {
        LOGE("%s-%d: sm2InterruptEnable failed, ret = %d\r\n", __func__, __LINE__, ret);
        goto end;
    }

    if (pDrvData->sbrCfg.regAddr == NULL) {
        LOGE("%s-%d: Invalid register address, devId=%d\r\n", __func__, __LINE__, devId);
        ret = -EINVAL;
        goto end;
    }
...
S32 sm2DoVerifyByPolling(DevList_e devId, U32 *dataIn, U32 *signBuf, U32 *pkey, U32 timeout)
{
    ...
    if (ret != EXIT_SUCCESS) {
        LOGE("%s-%d: sm2InterruptDisable failed, ret = %d\r\n", __func__, __LINE__, ret);
        ret = -EIO;
        goto end;
    }
    ...
    if (ret != EXIT_SUCCESS) {
        LOGE("%s-%d: sm2PollDone failed, ret = %d\r\n", __func__, __LINE__, ret);
        ret = -ETIMEDOUT;
        goto end;
    }
    ...
    if (ret != EXIT_SUCCESS) {
        LOGE("%s-%d: sm2PollResult failed, ret = %d\r\n", __func__, __LINE__, ret);
        ret = -EIO;
        goto end;
    }
...
S32 sm2DeInit(DevList_e devId)
{
    ...
    if (!isDrvInit(devId)) {
        LOGE("%s-%d: Device not initialized, devId=%d\r\n", __func__, __LINE__, devId);
        ret = -EIO;
        goto end;
    }
